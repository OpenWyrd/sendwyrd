...
| 2026-04-27 | **NIP-C6 (Capability-URL References) drafted + PR submitted**: scheme-agnostic NIP defining tag schema for Nostr events that reference capability-encrypted artifacts hosted off-Nostr. Generic — does not name SendWyrd. PR #2327 on `nostr-protocol/nips` is **OPEN, awaiting pre-discussion pings**. Spec at `what/docs/spec/nip_capability_url_v1.md`; submission script at `scripts/submit_nip_c6.sh` (re-runnable for any number renumbering); process skill at `how/skills/skill_nip_submission.md`. | NIP submission session |
