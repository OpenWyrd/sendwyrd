...
```bash
cd /tmp
gh secret set SENTRY_AUTH_TOKEN --repo DeltaClimbs/sendwyrd --body "$(cat ~/.config/sentry/auth_token)"
gh secret set SENTRY_ORG --repo DeltaClimbs/sendwyrd --body "sendwyrd"
gh secret set SENTRY_PROJECT_WEB --repo DeltaClimbs/sendwyrd --body "sendwyrd-web"
gh secret set SENTRY_PROJECT_API --repo DeltaClimbs/sendwyrd --body "sendwyrd-api"
```

Verified via `gh secret list --repo DeltaClimbs/sendwyrd`:
