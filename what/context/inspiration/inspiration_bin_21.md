...
**Why not v1**: burn-after-read fundamentally conflicts with SendWyrd's forward-chain use cases per VISION (the URL must work for the nth recipient who got it via Alice → Bob → Carol, not only for the first reader). For a pastebin, "first reader" is well-defined; for a relay primitive, it isn't.
