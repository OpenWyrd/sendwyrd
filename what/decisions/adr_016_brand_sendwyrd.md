...
- **GitHub repository renamed**: `github.com/DeltaClimbs/MOP` → `github.com/DeltaClimbs/sendwyrd`. GitHub serves automatic redirects from the old path; old clones can be updated with `git remote set-url origin https://github.com/DeltaClimbs/sendwyrd.git`.
...
- **Claude Code memory directory** at `~/.claude/projects/-home-michael-lattice-MOP/` was copied to `-home-michael-lattice-sendwyrd/` to preserve user-profile, feedback, and project memory across the path change. Old path retained until next session start verifies the new path works.
