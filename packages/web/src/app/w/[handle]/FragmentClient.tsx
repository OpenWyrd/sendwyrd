 ...
 *
 * The parent server component (page.tsx) SSR-fetches the envelope (or the
 * gone/not_found/error state) and passes it down via the `initial` prop.
 * This client component reads K_read from window.location.hash (which the
 * browser never transmits to servers, per RFC 3986), runs the decrypt,
 * resolves transitives, and renders. Eliminating the initial client→worker
 * round trip saves ~200-500ms of perceived latency.
 *
...
/**
 * Wire-shape of the SSR fetch result. The parent page.tsx fetches the
 * envelope server-side and passes one of these in. We then drive decrypt
 * + transitive resolution off the live envelope (no second fetch).
 */
export type InitialFetch =
  | { kind: "ok"; data: FetchEnvelopeResponse }
  | { kind: "gone"; reason: string; gone_at: string }
  | { kind: "not_found" }
  | { kind: "error" };

export default function FragmentClient({
  handle,
  initial,
}: {
  handle: string;
  ...
  const [state, setState] = useState<State>(() => {
    // Bootstrap state from the SSR fetch. We're still pre-decrypt at this
    // point — the actual ready/decrypt-failed transitions happen in the
    // useEffect below once we can read window.location.hash.
    if (initial.kind === "gone") {
      return { kind: "gone", reason: initial.reason, gone_at: initial.gone_at };
    }
    if (initial.kind === "not_found") return { kind: "not_found" };
    if (initial.kind === "error") return { kind: "network_error" };
    return { kind: "loading" };
  });
  ...
  useEffect(() => {
    if (!handle) return;
    const entry = listHistory().find((e) => e.handle === handle) ?? null;
    setHistoryEntry(entry);
  }, [handle]);

  useEffect(() => {
    let cancelled = false;
    if (initial.kind !== "ok") return; // terminal SSR state already in `state`
    ...
    (async () => {
      try {
        const handleBytes = b64uDecode(initial.data.handle);
        const k_read = b64uDecode(k_read_b64u);
        const body = await decryptFromBase64Url(initial.data.envelope, {
          k_read,
          handle: handleBytes,
          expires_at_ms: initial.data.expires_at,
